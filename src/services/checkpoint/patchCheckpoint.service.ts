...
import { ICheckinRequestUpdate, ICheckpointPost } from "../../interfaces/checkpoint";
...
const patchCheckpointService = async ({
  ...
  if (checkpoints.output) {
    const receivedDifference = calculateTime(splitHour(checkpoints.entry),splitHour(checkpoints.output), "dif")

    timeProject = calculateTime(splitHour(timeProject), splitHour(receivedDifference), "dif")
  }
  ...
  const newTime = calculateTime(splitHour(checkpoints.entry),splitHour(output), "dif")
